#include "MotoSearchResultDialog.h"
#include "../../View.h"

MotoSearchResultDialog::MotoSearchResultDialog(vector<pair<Moto, unsigned int>>* _result, View* _view, QWidget* _parent):
...

{
    ...
    setWindowTitle("MOTO DISPONIBILI");

    int i=0;
    for(auto it=result->begin(); it !=result->end(); ++it){
        ItemMoto* itemMoto=new ItemMoto(QString::fromStdString(it->first.getMake()),
                                         ...
                                         QString::fromStdString(it->first.getSidecar() ? "Si" : "No"),
                                         QString::fromStdString(it->first.getEngineProtection() ? "Si" : "No"),
                                         searchList);

        QListWidgetItem* lwi=new QListWidgetItem(searchList);
        lwi->setSizeHint(itemMoto->sizeHint());
        searchList->setItemWidget(lwi, itemMoto);
...
void MotoSearchResultDialog::removeVehicle() const { view->removeVehicle(); }

void MotoSearchResultDialog::removeDialog() const {
    searchList->close();
    delete result;
...
unsigned int MotoSearchResultDialog::returnVehicleForRemoveIndex() const {
...
void MotoSearchResultDialog::updateSearchResultDialog() const {

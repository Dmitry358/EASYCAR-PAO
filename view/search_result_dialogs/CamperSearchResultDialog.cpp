#include "CamperSearchResultDialog.h"

CamperSearchResultDialog::CamperSearchResultDialog(vector<pair<Camper, unsigned int>>* _result, View* _view, QWidget* _parent):
...

{
    ...
    setWindowTitle("CAMPER DISPONIBILI");

    int i=0;
    for(auto it=result->begin(); it !=result->end(); ++it){

        ItemCamper* itemCamper=new ItemCamper(QString::fromStdString(it->first.getMake()),
                                              ...
                                              QString::number(it->first.getSleepingPlaces()),
                                              QString::fromStdString(it->first.getToilet() ? "Si" : "No"),
                                              QString::fromStdString(it->first.getKitchen() ? "Si" : "No"),
                                              searchList);

        QListWidgetItem* lwi=new QListWidgetItem(searchList);
        lwi->setSizeHint(itemCamper->sizeHint());
        searchList->setItemWidget(lwi, itemCamper);
...
void CamperSearchResultDialog::removeVehicle() const {
    view->removeVehicle();
}

void CamperSearchResultDialog::removeDialog() const {
    searchList->close();
    delete result;
    view->closeSearchResultDialog();
}

unsigned int CamperSearchResultDialog::returnVehicleForRemoveIndex() const {
...
void CamperSearchResultDialog::updateSearchResultDialog() const {
